    ...
    <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center justify-end gap-2">
            @if ($projects->isNotEmpty())
                <button
                    type="button"
                    wire:click="selectVisible(@json($projects->pluck('id')->values()))"
                    class="rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm hover:bg-zinc-50"
                >
                    Select this page
                </button>
            @endif
            <label class="block">
                ...
                <input
                    ...
                    type="search"
                    placeholder="Search projects or source IDs"
                    class="w-72 max-w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm"
                />
            </label>
        </div>
    </div>
                    ...
                    <div class="flex flex-wrap items-end gap-2">
                        <label>
                            ...
                            <select
                                ...
                                class="h-10 min-w-56 rounded-md border border-violet-300 bg-white px-3 text-sm"
                            >
                        ...
                        <button
                            type="button"
                            wire:click="mergeSelected"
                            wire:confirm="Merge all selected projects? Their chats and source identifiers will be moved into the project you chose to keep."
                            class="h-10 rounded-md border border-violet-950 bg-violet-900 px-4 text-sm font-semibold text-white shadow-sm hover:bg-violet-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-700"
                        >
                            Merge selected
                        </button>
                        <button
                            ...
                            class="h-10 rounded-md border border-violet-300 bg-white px-3 text-sm text-violet-900 hover:bg-violet-100"
                        >
                            Clear
                        </button>
                    </div>
        ...
        <div class="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
            @foreach ($projects as $project)
                <article
                    ...
                    @class (['rounded-xl border bg-white p-5 shadow-sm', 'border-violet-400 ring-2 ring-violet-100' => in_array($project->id, $selectedProjectIds, true), 'border-zinc-200' => ! in_array($project->id, $selectedProjectIds, true)])
                >
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex min-w-0 items-start gap-3">
                            <input
                                ...
                                class="mt-1 rounded border-zinc-300 text-violet-700 focus:ring-violet-600"
                            />
                            <a

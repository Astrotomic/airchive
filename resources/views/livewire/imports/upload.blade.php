...
<div>
    ...
    <form wire:submit="save" class="mb-10 rounded-lg border border-zinc-200 bg-white p-6">
        <p class="mb-4 text-sm text-zinc-600">
            Upload ChatGPT JSON/ZIP exports, full Cursor export ZIPs, or individual Cursor transcript JSONL files.
        </p>

        <input type="file" wire:model="upload" accept=".json,.jsonl,.zip" class="block w-full text-sm">

        @error('upload')
            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
        @enderror

        <button type="submit" class="mt-4 rounded-md bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800" wire:loading.attr="disabled">
            Upload and import
        </button>
    </form>
                    ...
                    @foreach ($batches as $batch)
                        ...
                        @if ($batch->error_message)
                            <tr class="border-t border-zinc-100 bg-red-50">
                                <td colspan="4" class="px-4 py-3 text-red-700">{{ $batch->error_message }}</td>
                            </tr>
                        @endif
                    @endforeach
                </tbody>

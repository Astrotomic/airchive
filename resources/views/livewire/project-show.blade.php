    ...
    <div class="mt-2 grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <div class="min-w-0">
            <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-semibold break-words">{{ $project->display_name }}</h1>
                ...
                <div class="flex flex-wrap items-end gap-2">
                    <label>
                        ...
                        <input
                            ...
                            class="h-10 w-64 max-w-full rounded-md border border-zinc-300 bg-white px-3 text-sm"
                        />
                    </label>
                        ...
                        <label>
                            ...
                            >
                                ...
                                @foreach ($platforms as $platformCase)
                                    <option value="{{ $platformCase->value }}">{{ $platformCase->label() }} ({{ $platformCounts[$platformCase->value] }})</option>
                                @endforeach
                            </select>
                        </label>
                    @endif
            ...
            @if ($conversations->isEmpty())
                <div class="rounded-lg border border-dashed border-zinc-300 bg-white p-10 text-center text-zinc-500">{{ $platform !== '' || $search !== '' ? 'No conversations match the current search and filters.' : 'No conversations in this project.' }}</div>
            @else
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-2">
                        <label>
                            ...
                            <input
                                ...
                                maxlength="32"
                                placeholder="📁"
                                class="h-10 w-full rounded-md border border-zinc-300 px-3 text-center text-lg leading-none"
                            />
                        </label>
                        <label>
            ...
            <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                ...
                <div class="mt-3 space-y-2">
                    @foreach ($identifiers as $identifier)
                        <div
                            wire:key="identifier-{{ $identifier->id }}"
                            class="rounded-md bg-zinc-50 p-3"
                        >
                            ...
                            <p class="mt-2 font-mono text-[10px] break-all text-zinc-700">{{ $identifier->source_identifier }}</p>
                        </div>
                    @endforeach
                </div>
            </section>
            ...
            @if ($mergeCandidates->isNotEmpty())
                <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                    ...
                    >
                        ...
                        @error ('mergeProjectId')
                            <p class="text-xs text-red-600">{{ $message }}</p>
                        @enderror
                        <button
                            type="submit"
                            wire:confirm="Merge this project into {{ $project->display_name }}? Its source identifiers will be kept."

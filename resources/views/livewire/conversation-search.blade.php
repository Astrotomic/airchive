            ...
            @foreach ($results as $conversation)
                ...
                >
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div class="min-w-0">
                            <h2 class="font-medium break-words">{{ $conversation->title ?: 'Untitled conversation' }}</h2>
                            <p class="mt-1 text-xs text-zinc-500">{{ $conversation->source_platform->label() }} · {{ $conversation->last_message_at ? 'last message '.$conversation->last_message_at->diffForHumans() : 'message time unavailable' }}</p>
                        </div>

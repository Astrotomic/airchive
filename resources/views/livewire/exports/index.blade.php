...
<div>
    ...
    <form method="POST" action="{{ route('exports.download') }}" class="space-y-6">
        @csrf
        ...
        <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_12rem_18rem_auto] lg:items-end">
                <label>
                    ...
                    <input name="query" value="{{ $query }}" wire:model.live.debounce.300ms="query" type="search" placeholder="Titles and message content" class="h-10 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm">
                </label>
                <label>
                    <span class="mb-1 block text-xs text-zinc-500">Platform</span>
                    <select name="platform" wire:model.live="platform" class="h-10 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm">
                ...
                <label>
                    ...
                    <select name="project" wire:model.live="projectFilter" class="h-10 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm">
                ...
                @if ($conversations->isNotEmpty())
                    <div class="mt-3 grid gap-2 lg:grid-cols-2">
                        @foreach ($conversations as $conversation)
                            <a href="{{ route('conversations.show', $conversation) }}" target="_blank" class="rounded-md border border-zinc-100 px-3 py-2 hover:border-zinc-300">
        ...
        <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <div class="mt-4 grid gap-4 md:grid-cols-2">
                <label>
                    ...
                    <select name="mode" class="h-10 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm">
                ...
                <label>
                    ...
                    <select name="format" class="h-10 w-full rounded-md border border-zinc-300 bg-white px-3 text-sm">
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <button type="submit" @disabled($conversations->isEmpty()) class="rounded-md bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-40">
                Download ZIP
            </button>
        </div>
    </form>

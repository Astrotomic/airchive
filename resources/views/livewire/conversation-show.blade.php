...
<div class="grid min-w-0 max-w-full items-start gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
    <div class="min-w-0">
        <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
            <div class="min-w-0">
                <a href="{{ route('conversations.index') }}" class="text-sm text-zinc-500 hover:text-zinc-800">← Back</a>
                <h1 class="mt-2 break-words text-2xl font-semibold">{{ $conversation->title ?: 'Untitled conversation' }}</h1>
                <p class="mt-1 text-sm text-zinc-500">{{ $conversation->source_platform->label() }}</p>
            </div>
            <div class="flex flex-wrap items-center gap-3">
                <label class="flex items-center gap-2 text-sm">
                    <input type="checkbox" wire:model.live="collapseTools" class="rounded border-zinc-300">
                    Collapse tools
                </label>
                <label class="flex items-center gap-2 text-sm">
                    <input type="checkbox" wire:model.live="showBranches" class="rounded border-zinc-300">
                    Show branches
                </label>
                @foreach (ExportFormat::cases() as $format)
                    <a href="{{ route('conversations.export', [$conversation, 'format' => $format->value]) }}" class="rounded-md border border-zinc-300 px-3 py-2 text-sm hover:bg-zinc-50">{{ $format->label() }}</a>
                @endforeach
            </div>
        </div>
        ...
        <div class="space-y-3">
            ...
            >
                ...
                <div class="turn-card__body space-y-3 text-sm leading-relaxed">
                    @foreach ($turn['message_ids'] as $messageId)
                        <span id="message-{{ $messageId }}" class="block scroll-mt-6" aria-hidden="true"></span>
                    @endforeach
                    @foreach ($turn['blocks'] as $block)
                        <x-content-block :block="$block" :collapse-tools="$collapseTools" />
                    @endforeach
            ...
            @empty
                <div class="rounded-lg border border-dashed border-zinc-300 bg-white p-8 text-center text-zinc-500">
